...
def circumcircle(tri):
	"""
    Calculate the circumcircle of a triangle defined by three points.

    Parameters:
        tri (list of list of int/float): A list containing three points of the triangle, each represented as a list of two integers or floats.
            Example: [[x1, y1], [x2, y2], [x3, y3]]

    Returns:
        list: A list containing the center coordinates and radius of the circumcircle.
            Example: [[center_x, center_y], radius]

    Raises:
        ValueError: If the input triangle points are not in the correct format or if the triangle is degenerate (collinear points).
        ZeroDivisionError: If the determinant of the triangle's matrix is zero, indicating a possible divide by zero situation.
        TypeError: If the input points contain non-numeric elements.
...
def pointInCircle(point, circle):
	"""
    Check if a point lies inside a given circle.

    Parameters:
        point (list of int/float): A point represented as a list of two integers or floats.
            Example: [x, y]
        circle (list): A list containing the center coordinates and radius of the circle.
            Example: [[center_x, center_y], radius]
    ...
    Raises:
        ValueError: If the input point or circle is not in the correct format or if the radius is negative.
        TypeError: If the input point or circle contains non-numeric elements.

    Example:
        >>> pointInCircle([1, 1], [[0, 0], 2])
        True
    """
...
class Point():
    ...
    Methods:
        pos(): Returns the position of the point as a list [x, y].
        isEqual(other_point): Determines if two points are equivalent.
        pointToStr(): Converts the point into a string for debugging purposes.
...
class Edge():
    ...
    Methods:
        isEqual(other_edge): Tests if two edges are equivalent.
        edgeToStr(): Converts the edge into a string for debugging purposes.
        length(): Calculates the length of the edge.
        edgeIntersection(other_edge): Determines if two edges intersect.
...
class Graph():
	"""
    Represents a graph structure for handling points, edges, and triangles.

    Methods:
        addPoint(point): Adds a point to the graph.
        addEdge(edge): Adds an edge to the graph.
        addTriangle(triangle): Adds a triangle to the graph.
        triangleIsDelaunay(triangle): Tests if a triangle is Delaunay.
        generateDelaunayMesh(): Generates a Delaunay mesh based on the current graph.
